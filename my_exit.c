...
int _mycd(info_t *info)
{
	...
	if (!info->argv[1])
	{
		...
		if (!target_dir)
			chdir_result = chdir((target_dir = _getenv(info, "PWD=")) ?
					target_dir : "/");
		else
	...
	else if (_strcmp(info->argv[1], "-") == 0)
	{
		...
		chdir_result = chdir((target_dir = _getenv(info, "OLDPWD=")) ?
				target_dir : "/");
	}
	else
	...
	if (chdir_result == -1)
	{
		...
		print_input(info->argv[1]), _eputchar('\n');
	}
	else
	{
